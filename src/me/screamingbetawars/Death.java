...
public class Death extends EntityListener implements Listener {
    ...
    public void onDamage(EntityDamageEvent event) {
        if (Game.isGameNPC(event.getEntity().getEntityId())) {
    ...
    public void onEntityHit(EntityDamageByEntityEvent event) {
        if ((event.getEntity() instanceof Player) && (event.getDamager() instanceof Player)) {
            if (Game.getPlayerTeam(((Player) event.getEntity()).getName()).equals(Game.getPlayerTeam(((Player) event.getDamager()).getName()))) {
                event.setCancelled(true);
            }
        }
    }
    ...
    public void onKill(EntityDeathEvent event) {
        if (event.getEntity() instanceof Player) {
            ...
            if (Game.isPlayerPlaying(name)) {
                ...
                if (Game.stopIfEmpty(game2, game)) {
                    event.getDrops().clear();
                }
            }
        }
